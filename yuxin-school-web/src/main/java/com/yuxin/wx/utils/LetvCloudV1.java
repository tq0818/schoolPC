...
public class LetvCloudV1 {
    ...
    public String doGetApi(String url,String authorization) throws UnsupportedEncodingException {
        HttpClient client = new HttpClient(); // 实例化httpClient
        HttpMethod method = new GetMethod(url); //
        String responseContent = "";
        try {
            method.setRequestHeader("Authorization","Basic "+Base64.encode(authorization.getBytes()));
            method.setRequestHeader("Lecloud-api-version","0.4");
            client.executeMethod(method); // 执行
