	...
	<li class="teacher clear">
        ...
        <div class="r-list-content clear hide">
             <div class="r-list-content-title">
                 ...
                 <a href="<%=request.getContextPath()%>/teacherManger/updateOrAddTeacher/${item.id}/${companyId}" class="btn btn-link btn-mini">编辑</a>
             ...
             <p class="c">
                 ...
                 <span class="c-content">
                     <%--<input type="text" class="readonly"
                            value="<c:choose>
                                        <c:when test="${item.educationCode eq 'UNDER_JUNIOR'}">大专以下</c:when>
                                        <c:when test="${item.educationCode eq 'JUNIOR'}">大专</c:when>
                                        <c:when test="${item.educationCode eq 'BECHELOR'}">本科</c:when>
                                        <c:when test="${item.educationCode eq 'POSTGRADUATE'}">研究生</c:when>
                                        <c:when test="${item.educationCode eq 'DOCTOR'}">博士生及以上</c:when>
                                        <c:otherwise>大专以下</c:otherwise>
                            </c:choose>" readonly>--%>
                     <input type = "text" value="${item.educationCode}"  class="readonly" readonly="readonly">
                 </span>
             </p>
             <p class="c">
