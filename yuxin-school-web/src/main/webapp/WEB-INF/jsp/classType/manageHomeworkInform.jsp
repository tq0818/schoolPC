...
<div class="u-wrap company overflow">
    ...
    <div class="right-side">
        <div>
            <div class="title-box">
                <div class="tit-font">
                    <span class="t">课后作业</span>
                </div>
            </div>
           
	       <div class="classTra" style="margin-top: 15px;margin-right: 60px;">
			说明：开启以下通知方式后，学员会作业被批阅后收到相应的通知
		   </div>
			   <div class="c-main" style="width:95%;margin: 20px 0px 10px 50px;">
				<div class="s-title" style="padding-bottom:10px; padding-left:13px;">
					<h2 class="h6" style="font-size: 14px;">短信通知:<span class="kg kgCom" level="com"><i class="iconfont close" id="courseProtocolButton">&#xe604;</i><span class="i close" style="font-size:14px;" id="courseText">&nbsp;&nbsp;已禁用</span></span></h2>
				</div>
				<div class="s-title" style="padding-bottom:10px; padding-left:13px;">
					<h2 class="h6" style="font-size: 14px;">邮件通知:<span class="kg kgItem" level="item"><i class="iconfont close" id="packageProtocolButton">&#xe604;</i><span class="i close" style="font-size:14px;" id="packageText">&nbsp;&nbsp;已禁用</span></span></h2>
				</div>
			</div>
           <div class="clear"></div>
        </div>
		...
		<script type="text/javascript">
			$(function() {
				...
				$selectSubMenus("course_homework_inform");
				if($(".tiHeader .navspace li").length == 1){
				 ...
				 $.ajax({
					 url:rootPath+"/companyFunctionSet/queryCompanyHomeworkExist",
					 type:"post",
					 data:{"functionCode":"HOMEWORK_MESSAGE_INFORM"},
					 dataType:"json",
				 ...
				 $.ajax({
					 url:rootPath+"/companyFunctionSet/queryCompanyHomeworkExist",
					 type:"post",
					 data:{"functionCode":"HOMEWORK_EMAIL_INFORM"},
					 dataType:"json",
					 success:function(jsonData){
						 if(jsonData && jsonData == "success"){
				 ...
				 $('#courseProtocolButton').on('click',function(){
					 if($(this).hasClass('open')){
						 ...
						 $('#courseText').html('&nbsp;&nbsp;已禁用');
						 $.ajax({
							 url:rootPath+"/companyFunctionSet/saveOrUpdataHomeworkInform",
							 type:"post",
							 data:{"functionCode":"HOMEWORK_MESSAGE_INFORM","status":"0"},
							 dataType:"json",
							 success:function(jsonData){
							 } 
						 })
						 ...
						 $.ajax({
							 url:rootPath+"/companyFunctionSet/saveOrUpdataHomeworkInform",
							 type:"post",
							 data:{"functionCode":"HOMEWORK_MESSAGE_INFORM","status":"1"},
							 dataType:"json",
							 success:function(jsonData){
							 } 
						 })
				 ...
				 $('#packageProtocolButton').on('click',function(){
					 if($(this).hasClass('open')){
						 ...
						 $.ajax({
							 url:rootPath+"/companyFunctionSet/saveOrUpdataHomeworkInform",
							 type:"post",
							 data:{"functionCode":"HOMEWORK_EMAIL_INFORM","status":"0"},
							 dataType:"json",
							 success:function(jsonData){
							 } 
						 })
						 ...
						 $.ajax({
							 url:rootPath+"/companyFunctionSet/saveOrUpdataHomeworkInform",
							 type:"post",
							 data:{"functionCode":"HOMEWORK_EMAIL_INFORM","status":"1"},
							 dataType:"json",
							 success:function(jsonData){
							 } 
						 })
