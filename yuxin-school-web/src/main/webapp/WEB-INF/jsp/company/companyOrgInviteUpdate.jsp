...
<script>
  	...
  	$(function(){
  		...
  		$.ajax({
			 type:"POST",
			 url :rootPath + "/companyInvitConfigOrg/ajaxCheck",
			 success:function(data){
				 ...
				 if(data&&data.rewardsCustomSetting){
					 $('#rewardsCustomSetting').removeClass('close').addClass('open').html('&#xe603;').next('span').eq(0).removeClass('close').addClass('open').html('&nbsp;&nbsp;已启用');
				 }
				 ...
				 if(data&&data.invitCastAwardFlag){
					 $('#tichenglist').find('.chet').eq(0).prop('checked','checked').siblings('.numberday').removeAttr('disabled').val(data.invitCastAward);
				 }else if(data && data.invitCastAwardFlag==0){
					 $('#tichenglist').find('.chet').eq(1).prop('checked','checked').siblings('.numberday').removeAttr('disabled').val(data.invitCastAward);
				 }
...
<body >
	 ...
	 <div class="right-side wxRight-side">
            <div>
                ...
                <div class="footerSetBox footerSetBox-fen">
                    <h4>说明：开启该功能后，可以为代理机构发放二维邀请码，代理机构通过邀请码邀请学生成功后，可获得相应的奖励。</h4>
                    ...
                    <div class="contentList">
                        ...
                        <div class="inviteList-box " id="tichenglist">
                            ...
                            <div class="inviteList inviteList-uncheck">
                                ...
                                <span>被邀请人每次消费代理机构可获得 :</span>
                                <span class="coupon">提成</span>
                                <input type="text" class="numberday ticheng" maxlength="11"/>
                                <span class="yuan">%</span>
                            </div>
                             <div class="inviteList inviteLmb">
                                <div class="title-box titlefen-box titleC-box borSpe">
                                    <div class="tit-font titfen-font">
                                        <span class="t ts">提成比例个性化设置</span>
                                        <span class="kg kgCla kg-fen" level="cla">
                                        <i class="iconfont close" id="rewardsCustomSetting"></i><span class="i fs12">&nbsp;&nbsp;已禁用
                                         </span>
                                        </span>
                                        <em>说明：开启后在“代理机构管理”中，可以对任意一个代理的提成比例进行单独配置。</em>
                                    </div>
                                </div>
                            </div>
                        </div>
	...
	<script src="<%=rootPath%>/javascripts/company/inviteOrg.js"></script>
