...
<div class="u-wrap student">
	<div class="mainbackground" style="height:115px;">
					...
					<div class="m-b-p-right">
						<div class="btns">
							<a href="javascript:;" id="1" onclick="returnFee('${payMaster.applyChannelCode}');"
								...
								onclick="detail();" class="m-btn m-mb">详情</a> <a
								href="javascript:;" id="5" onclick="changeClassNo('${payMaster.applyChannelCode}');"
								class="m-btn m-mb change">更换班号</a>
						</div>
					</div>
				</div>
			</div>
...
<form action="<%=request.getContextPath()%>/student/full/selectEntryDetail"
...
<script>
	...
	function changeStudent(code){
		...
		//$("#payMasterId").val(id);
		var src="<%=request.getContextPath()%>/student/full/changeStudent";
	...
	function returnFee(code){
		...
		//$("#payMasterId").val(id);
		var src="<%=request.getContextPath()%>/student/transaction/full/returnFee";
		$("#toForm").attr("action",src)[0].submit();
	}
	function changeClass(code){
		...
		//$("#payMasterId").val(id);
		var src="<%=request.getContextPath()%>/student/transaction/full/changeClass";
		$("#toForm").attr("action",src)[0].submit();
	}
	function changeClassNo(code){
		if(code == 'CHANNEL_STUDYCARD'){
			$.msg("学习卡兑换的课程不能做异动操作！");
			return false;
		} 
		//$("#payMasterId").val(id);
		var src="<%=request.getContextPath()%>/student/transaction/full/changeClassNo";
		$("#toForm").attr("action", src)[0].submit();
	}
</script>