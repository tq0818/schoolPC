...
//			})
		...
		init2: function(callback){
			...
			//初始化下拉框
			$.ajax({
				url:rootPath+"/classPackageStage/queryByPackageId",
				data:"classPackageId="+$(".package").val(),
				type:"post",
				dataType:"josn",
				success:function(jsonData){
					$(".packageStage").html('').append('<option>全部</option>');
					$.each(jsonData,function(i,data){
						$(".packageStage").append('<option value="'+data.id+'">'+data.name+'</option>');
					})
				}
			})
			//是否代报考
...
//							    				$this.getVideos();
		...
		save2: function(successBack,errorBack){
			...
			payMaster.commodityId=$(".package").val();
			...
			$.ajax({
				url: rootPath+"/classPackage/savePayMasterMany",
				...
				success : function(payMaster){
					...
					if($this.options.ns){
	    				$.alert("报名成功，同时为该学员生成了网校的账户，账户名称是学员的手机号，初始密码是手机号后六位，已通过短信形式通知学员",function(){
		    				location.href=rootPath+"/classPackage/studentList/"+$(".package").val();
		    			})
	    			}else{
