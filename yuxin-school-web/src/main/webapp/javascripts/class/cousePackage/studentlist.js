    ...
    var student = {
        init: function () {
            ...
            $(".importexcle").on('click', function () {
                $(this).attr("href", rootPath + "/classPackage/importPage/"+$("#packageId").val());
            });
            // 导出用户
            $(".exportexcle").on(
                'click',
                function () {
                    if ($("#tableList").find("tr").eq(1).find("td").length <= 1) {
                        ...
                        $("#searchForm").attr("action",
                            rootPath + "/classPackage/exportExcle")
                            .submit();
                    }
        ...
        search: function (page) {
            ...
            data.commodityId=$("#packageId").val();
...
//            }
            ...
            $.ajax({
                    url: rootPath + "/classPackage/queryStudentsList",
                    data: data,
                    type: 'post',
...
//                                    + '</td>'
                                    ...
                                    + '</td>'
                                    + '<td class="slink">'
                                    + '<a stuId="' + stu.id + '" href="'+rootPath+'/stuLeanRecord/stuClassPackageLeanRecord/'+$("#packageId").val()+'/'+stu.id+'">学习记录</a>|'
                                    + '<a class="more" href="javascript:void(0);">更多</a>'
                                    + '<ul class="none">'
//                                    + (stu.paymaterCount > 0 ? '<li><a class="toTransaction" mobile="' + stu.mobile + '" href="javascript:void(0);">异动</a></li>' : '')
                                    ...
                                    + '<li><a class="delteStuOrder" stuId="' + stu.id + '" href="javascript:void(0);">移出课程包</a></li>'
                                    + (stu.protocolId ? '<li><a class="protocolDetail" data-protocolid="' + stu.protocolId + '" href="/courseProtocolConfig/toProtocolDetail?protocolId='+ stu.protocolId +'&classPackageId='+$("#packageId").val()+'&stuId='+stu.id+'"  target="blank">课程包协议</a></li>':"")
            ...
            $(".user-list").on("click", ".showSignUp", function () {
                ...
                .on('click','.delteStuOrder',function(){
                	...
                	$.confirm("您确认要移出此课程吗",function(b){
                		if(b){
                        	var commodityId=$("#packageId").val();
                        	 $.ajax({
                                 type: 'post',
                                 url: rootPath + "/classPackage/deleteStuPaymaster",
                                 data: {id: stuId,commodityId: commodityId},
                                 dataType: 'json',
                                 success: function (jsonData) {
                                     if (jsonData == "success") {
                                    	 $.msg("该学员已移出此课程包",function(){
                                    		 //检查学员是否有其他订单，没有则取消vip_flag
                                    		 $.ajax({
                                                 type: 'post',
                                                 url: rootPath + "/classPackage/queryStupaymater",
                                                 data: {stuId: stuId},
                                                 dataType: 'json',
                                                 success: function (jsonData) {
    ...
    var Form = {
        init: function () {
            ...
            $(".addStudent").on('click', function () {
                ...
                var form = document.createElement("form");
                document.body.appendChild(form);
                form.action = rootPath + '/classPackage/showSignUp';
                form.method = "post";
                form.target = "_self";
                var fileName = document.createElement("input");
                fileName.type = 'hidden';
                fileName.value = commodityId;
                fileName.name = 'commodityId';
                form.appendChild(fileName);
                form.submit();
            });
                ...
                $.ajax({
                    ...
                    success: function (jsonData) {
                        ...
                        if (jsonData.identityTypeCode == '' || jsonData.identityTypeCode == null) {
                            $("#uIdentity").prop('disabled', true);
                        } else {
                            $("#uIdentity").prop('disabled', false);
                        }
