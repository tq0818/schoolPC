	...
	var student = {
		init : function() {
            ...
            $(".importexcle").on('click', function () {
            	window.location.href=rootPath + "/otherSchool/importStudents/"+$("#classTypeId").val()+"/"+$("#lableType").val();
            });
			// 导出用户
			$(".exportexcle").on(
							'click',
						...
						success : function(jsonData) {
							...
							$.each(jsonData.data,
											...
											'<tr data-buy="'+(stu.paymaterCount>0)+'">' 
													...
													+ '</td>'
													+ '<td class="slink">'
													+ '<a mobile="'+stu.id+'" href="'+rootPath+'/student_detail/classLeanRecord/'+stu.id+'/'+$("#classTypeId").val()+'/'+$("#lableType").val()+'">学习记录</a>|'
	...
	var Form = {
		init : function() {
			...
			$(".addStudent").on('click', function() {
                ...
                var form = document.createElement("form");
                document.body.appendChild(form);
                form.action = rootPath + '/otherSchool/showSignUp';
                form.method = "post";
                form.target = "_self";
                var fileName = document.createElement("input");
                fileName.type = 'hidden';
