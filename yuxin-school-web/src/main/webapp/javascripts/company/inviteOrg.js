...
$(function(){
	...
	$('#1thbtn').click(function(){
		if($(this).hasClass('close')){
			$.ajax({
				type:"POST",
				data:{"flag":"1"},
				url:rootPath+"/companyInvitConfigOrg/updateBtn",
	...
	$('#rewardsCustomSetting').click(function(){
		if($(this).hasClass('close')){
			$(this).removeClass('close').addClass('open').html('&#xe603;').next('span').eq(0).attr('class','i open fs12').html('&nbsp;&nbsp;已启用');
		}else{
			$(this).removeClass('open').addClass('close').html('&#xe604;').next('span').eq(0).attr('class','i fs12').html('&nbsp;&nbsp;已禁用');
		}
	})
    ...
    $('.btn-success').on('click',function(){
    	...
    	if($('#tichenglist').find('.chet').eq(0).is(':checked')){
    		...
    		if(!invitCastAward){
    			errorMsg="代理机构提成金额不能为空！";
    			$.alert(errorMsg);
    			return;
    		}
    		if(!reg2.test(invitCastAward)){
    			errorMsg = '请设置正确的代理机构提成金额（正数保留两位小数）';
    			$.alert(errorMsg);
    			return;
    		}
    	}
    	...
    	data.rewardsCustomSetting=rewardsCustomSetting;
    	$.ajax({
			
			 type:"POST",
			 url :rootPath + "/companyInvitConfigOrg/saveOrUpdateCompanyInvitConfigOrg",
			 data :data,
			 success:function(data){
				 if(data == 'success')
					 $.msg('操作成功');
