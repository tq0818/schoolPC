(function ($) {
	...
	$('.table').on("click",".Withdraw",function(){
		...
		var proxyOrganId = $(this).attr('mark');
		var data = {};
		data.proxyOrganId = proxyOrganId;
		$.ajax({
			type:'get',
			url:rootPath+'/organInviteRewardRecord/todetaillistpage',
			data:data,
			success:function(jsondata){
					//$(".detailsPop").find('.teacz').text(jsondata.teacher.name?data.totalMoney.toFixed(2):0);
					$(".detailsPop").find('.teacz').text(jsondata.proxy.name?jsondata.proxy.name:"");
					...
					$('.hiddendetailteacherId').val(proxyOrganId);
					$(".detail-Parent").show();
    ...
    var rewardsrecorddetail = {
        ...
        search: function (page) {
            var $this = this;
            var data = {};
            var proxyOrganId = $('.hiddendetailteacherId').val();
            data.proxyOrganId = proxyOrganId;
            ...
            $.ajax({
                    url: rootPath + "/organInviteRewardRecord/queryRewardsRecordDetailList",
                    data: data,
                    type: 'POST',
