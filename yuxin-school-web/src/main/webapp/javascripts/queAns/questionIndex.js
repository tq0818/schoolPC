...
(function($){
	var Form={
			init : function(){
				...
				$(".itemSec").addClass("none");
				//--4.2
				var _id = Form.getUrlParam('i');
				if(_id){
					Form.findQuestion(1,oneId,twoId,jz,null,_id);
				}else{
					Form.findQuestion(1,oneId,twoId,jz);
				}
			},
			queryItemSecond : function (id){
				...
				$.ajax({
					...
					success : function(result) {
						if(result.length>0){
							$("#itemSecondList").html('');
							$("#itemSecondList").append("<a href='javascript:void(0);' class='btn sec btn-mini btn-default btn-success' ids=''>全部</a>");
							$.each(result,function(i,item){
								$("#itemSecondList").append("<a href='javascript:void(0);' class='btn sec btn-mini btn-default' ids='"+item.id+"'>"+item.itemName+"</a>");
							});
							$(".itemSec").removeClass("none");
			...
			findQuestion : function(pageNo,oneId,twoId,jz,questionName,_id){
				...
				$.ajax({
					...
					success : function(data) {
						...
						$(".plContent").on("click",".fbQue",function(){
							...
							if(plCon.length>500){
								$('<div class="c-fa">'+ "评论内容不能超过500个字" +'</div>').appendTo('body').fadeIn(100).delay(1000).fadeOut(200,function(){$(this).remove();
								});
								return;
							}
			...
			getUrlParam : function (paraName) { //此方法从home-page.js取得,reason :204 $.getUrlParam not a function
