...
//				resizeWindow($("div.step0").height()+50);
		...
		init0: function(callback){
			...
			$.ajax({
				...
				success: function(jsonData){
					...
					$(".list-infos").cityselect({
					    ...
					    prov:$this.options.student.province, //省份 
					    city:$this.options.student.city,     //市
					    dist:$this.options.student.county,
			...
			this.getExpandField(".step0 .mark-more","w",function(){
				$this.getExpandData(".step0 .mark-more","student","stuId",$this.options.student.id);
			});
			if(callback){
				callback();
			}
		...
		init1: function(callback){
			...
			this.getExpandField(".step1","r",function(){
				$this.getExpandData(".step1","student","stuId",$this.options.student.id);
			});
			if(callback){
				callback();
			}
		...
		getExpandField:function(ele,rw,func){
			$.ajax({
				...
				success:function(jsonData){
					var valueble;
					if(jsonData){
						$(ele).find(".main-content:last").after('<div class="main-content"><div class="m-title"><h2 class="h6">其他信息</h2></div><ul id="expandFields" class="list-infos clear"></ul></div>');
					}
