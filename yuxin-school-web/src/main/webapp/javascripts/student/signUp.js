...
//				resizeWindow($("div.step0").height()+50);
		...
		init0: function(callback){
			...
			if($("#mobileOrUsername").val()=="mobile"){
			$.ajax({
				...
				success: function(jsonData){
					...
					$(".list-infos").cityselect({
						url:rootPath + "/javascripts/company/city.min.js",
					    prov:$this.options.student.province?$this.options.student.province:"", //省份 
					    city:$this.options.student.city?$this.options.student.city:"",     //市
					    dist:$this.options.student.county?$this.options.student.county:"",
					    nodata:"none" //当子集无数据时，隐藏select
					});
					$(".step0").find(".addressDetail").val($this.options.student.addressDetail);
				}
			})
			}else{
				$.ajax({
					url : rootPath+"/student/searchByUsername1/"+$("#_userName").val(),
					type: "post",
					dataType :"json",
					success: function(jsonData){
						//去掉空值
			...
			this.getExpandField(".step0 .mark-more","w",function(){
				$this.getExpandData(".step0 .mark-more","student","stu_id",$this.options.student.id);
			});
		...
		init1: function(callback){
			...
			this.getExpandField(".step1","r",function(){
				$this.getExpandData(".step1","student","stu_id",$this.options.student.id);
			});
			if(callback){
				callback();
			}
...
//							    				$this.getVideos();
		...
		save0: function(successBack,errorBack){
			var $this=this;
			if($this.options.student.id){
				//修改&id="+$this.options.student.id+
				var data=$("#studentForm").getParams()+"&mobile="+$(".mobile").val()+"&id="+$this.options.student.id;
			}else{
				//新增
				...
				success: function(fixed){
					...
					if(fixed){
						$.confirm("该学员已经报名过这个班型，是否要继续？",function(e){
							if(e){
								$.ajax({
									url: rootPath+"/studentPayMaster/savePayMaster",
									data: $this.options.payMaster,
									type: "post",
									dataType: "json",
									async: false,
									...
									success : function(payMaster){
										...
										if($this.options.ns){
						    				$.alert("报名成功，同时为该学员生成了网校的账户，账户名称是学员的手机号，初始密码是手机号后六位，已通过短信形式通知学员",function(){
							    				location.href=rootPath+"/student/studentList";
							    			})
						    			}else{
...
//			});
		...
		//查询扩展字段
		getExpandField:function(ele,rw,func){
					...
					$.each(jsonData,function(i,data){
						...
						if(rw=="r" && data.orgAllowRead ){
							...
							if(data.tldType=="text"){
								...
								'<input type="text" id="'+data.colName+'" name="'+data.colName+'" style="'+(data.styleCss?data.styleCss:'')+'" class="'+(data.styleClass?data.styleClass:'')+'" value="'+(data.colValue?data.colValue:"")+'" >'+
								'</span>'+
								'</p></li>';
		...
		//查询扩展数据
		getExpandData:function(ele,table,colName,colValue){
			var data={};
