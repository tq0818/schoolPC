(function ($) {
    var inviteTeacher = {
        ...
        search: function (page) {
            ...
            $('#table1').find('tbody').find('tr:gt(0)').remove();
            $.ajax({
                    url: rootPath + "/teacherInviteRewardRecord/queryRewardsRecordList",
                    data: data,
                    type: 'post',
                    ...
                    success: function (jsonData) {
                        ...
                        $.each(jsonData.data,function (i, teac) {
                        	$('#table1').find('tbody').append('<tr class="tab-dtr">'+
                                    '<td>'+(teac.name?teac.name:"")+'</td>'+
                                    '<td>'+(teac.mobile?teac.mobile:"")+'</td>'+
                                    '<td>'+(teac.inviteCode?teac.inviteCode:"")+'</td>'+
                                    ...
                                    '<td><span class=" Withdraw" mark="'+(teac.teacherId)+'">详情</span></td>'+
                                '</tr>');
                            });
                        $("#pageNo").remove();
                        $("#table1").after('<input type="hidden" id="pageNo" value="'+jsonData.pageNo+'"/>');
                        $('.hidSortType').val('');
                    	$('.hidSort').val('');
    ...
    $(document).ready(function(){
    	...
    	$(".export").on('click',function(){
    		if ($("#table1").find("tbody").find("tr").length <= 1) {
                ...
                var page = $('#pageNo').val();
                page = page ? page : 1;
                window.location.href=rootPath+"/teacherInviteRewardRecord/exportExcel?name="+teacherName+"&date="+searchDateType1+"&page="+page
