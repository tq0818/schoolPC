		...
		function drawLinks() {
			...
			var appendItem = function(page_id, appendopts){
				...
				if(page_id == current_page){
					if(current_page==0 && isNaN(appendopts.text)){
						var lnk = jQuery("<li class='disabled'><a>"+(appendopts.text)+"</a></li>");
					}else if(current_page==np-1 && isNaN(appendopts.text)){
						var lnk = jQuery("<li class='disabled'><a>"+(appendopts.text)+"</a></li>");
					}else{
				...
				else
				{
					...
					lnk=jQuery("<li></li>").append(lnk);
				}
				if(appendopts.classes){lnk.addClass(appendopts.classes);}
				panel.append(lnk);
			}
			if(current_page<np){
				panel.append("<li><span>共"+maxentries+"条</span></li>");
			}else{
				var lastrows=maxentries%opts.items_per_page;
				panel.append("<li><span>共"+maxentries+"条</span></li>");
			}
