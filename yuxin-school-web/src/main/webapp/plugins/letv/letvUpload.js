...
window.html5Upload = (function () {
    ...
    (function () {
        ...
        (function ($) {
            ...
            $.fn.uploadletv = function (options) {
                uploadOption = $.extend(defaults, options || {});
                return this.each(function () {
                    $(this).change(function (data) {
                        //验证选择的文件格式
                        videoUpload.init(data);
                    });
                });
            };
        })(jQuery);
    ...
    var xhrEventCallback = (function () {
        ...
        var getFileKey = function (file) {
            return [getFileType(file), file.size,  (file.lastModifiedDate==null)?0:file.lastModifiedDate.getTime()].join('_');
        };
        ...
        xhrEventCallback.load = function (e) {
            ...
            if (res.transferedsize && res.transferedsize == res.totalsize) {
                //文件上传完成
                uploadOption.uploadFinish({ code: 0, video_unique:html5Upload.exportObject.selectFile.video_unique,msg: "上传完成",filesize:html5Upload.exportObject.selectFile.file.size,video_id:html5Upload.exportObject.selectFile.video_id },getFileKey(html5Upload.exportObject.selectFile.file));
                //uploadCookie.removeCookie(html5Upload.exportObject.selectFile.fileKey);
                videoUpload.xhrAbort();

                html5Upload.exportObject.selectFile.fileList.splice(0,1);
                if(html5Upload.exportObject.selectFile.fileList.length>0){
                    html5Upload.exportObject.selectFile.file = html5Upload.exportObject.selectFile.fileList[0].file;
                    html5Upload.exportObject.selectFile.fileKey = html5Upload.exportObject.selectFile.fileList[0].fileKey;
        ...
        xhrEventCallback.progress = function (e) {
            ...
            rate = rate / 1024;
            var s = html5Upload.exportObject.selectFile.file.size/1024;
            var fileSize = s > 1024 ? (((s / 1024 * 10) >> 0) / 10).toFixed(1) + "M" : (((s * 10) >> 0) / 10).toFixed(1) + "KB";
            ...
            if (isNaN(pc)) {
                ...
                uploadOption.uploadProgress({ progress: pc + "%", speed: rate, size:fileSize },getFileKey(html5Upload.exportObject.selectFile.file));
            }
        };
    ...
    var videoUpload = (function () {
        ...
        videoUpload.init = function(data){
            //验证文件的是否满足
            //允许上传的文件类型
            ...
            for(var i=0;i<data.target.files.length;i++){
                ...
                if (file.size < 0) {
                    uploadOption.uploadError({ code: 100, msg: file.name+"文件大小为0" });
                    return;
                } else if (!eval("/" + fileTypes + "$/i").test(fType)) {
                    uploadOption.uploadError({ code: 101, msg: file.name+"不支持此视频格式" });
                    return;
                } else if($("#"+getFileKey(data.target.files[i])).is("tr")){
                    uploadOption.uploadError({ code: 102, msg: file.name+"此视频已经存在" });
                    return ;

                }else{
                    //addFileOption.addFile({ code: 0, msg: "成功", data: { fileName: file.name, fileSize: file.size, fileType: fType } });
                    html5Upload.exportObject.selectFile.fileList = html5Upload.exportObject.selectFile.fileList || [];
                    html5Upload.exportObject.selectFile.fileList.push({"file":file,fileKey:getFileKey(file)});
                    //html5Upload.exportObject.selectFile.file = file;
                    //html5Upload.exportObject.selectFile.fileKey = getFileType(file);
                }

            }
            uploadOption.uploadBefore(data);

            //html5Upload.exportObject.selectFile.file = data.target.files[0];
            //html5Upload.exportObject.selectFile.fileKey = getFileKey(data.target.files[0]);
            if(!signUploadFlag){
                html5Upload.exportObject.selectFile.file = html5Upload.exportObject.selectFile.fileList[0].file;
                html5Upload.exportObject.selectFile.fileKey = html5Upload.exportObject.selectFile.fileList[0].fileKey;
                videoUpload.tryUpload();
        ...
        videoUpload.streamUpload = function (loadedsize) {
            ...
            html5Upload.exportObject.selectFile.nextUpload = videoUpload.nextUpload;
        ...
        videoUpload.nextUpload = function () {
            html5Upload.exportObject.selectFile.fileList.splice(0,1);
            if(html5Upload.exportObject.selectFile.fileList.length>0){
                html5Upload.exportObject.selectFile.file = html5Upload.exportObject.selectFile.fileList[0].file;
                html5Upload.exportObject.selectFile.fileKey = html5Upload.exportObject.selectFile.fileList[0].fileKey;
            ...
            try {
                uploadAjax.post(initUrl, function (data) {
                    ...
                    if (data.code == 0) {
                        ...
                        //html5Upload.exportObject.selectFile.video_id = data.data.video_id+"@@"+data.data.video_unique;
    ...
    var uploadFileOperation = (function () {
        ...
        //获取文件的key
        var getFileKey = function (file) {
            return [uploadFileOperation.getFileType(file), (file.lastModifiedDate==null)?0:file.lastModifiedDate.getTime()].join('_');
        };
        //选中文件的时候
